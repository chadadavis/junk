...
# Where the pages are linked
my $website = "http://www.mips.gsf.de";
my $webroot = "${website}/~davis/api/hepp";
# Where the code comes from
my $approot = "$ENV{HOME}/doc/dev/hepp";
# Where the docs go to
my $docroot = "$ENV{HOME}/public_html/api/hepp";

# See 'Global variables' below for other important settings

# Let the group write the files
#umask 0002;
...
# Init globals
$VERSION = do { my @r = (q$Revision: 679 $ =~ /\d+/g); sprintf "%d."."%02d" x $#r, @r };
$RELEASE = '1.0';
...
my @xl = ( 
	[ 
#		"${perllib}/bioperl-live/Bio", 
#		"http://doc.bioperl.org/releases/bioperl-1.4/Bio" 
#		"${website}/documentation/Bio" 
	] 
		);
...
if (! -d $target) {
    print STDERR "Target dir $target is not a directory.\n";
    exit;
}
...
# Just give the site redirector
print "Location: ${website}/documentation\n\n";
...
sub Generate_modules_index {
    ...
    # TODOBUG this is garbage, how did this not get caught -- Chad Davis
    #$renderer->url($tree->url());
    # Here's what it ought to look like
    $renderer->url($tree->url() . $tree->path_separator() . 
                   $node->path  . $tree->path_separator());
